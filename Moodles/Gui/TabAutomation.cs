...
public static class TabAutomation
{
    ...
    private static void DrawSelected()
    {
        ...
        if(Editing)
        {
            ...
            ImGui.SameLine();
            ImGui.SetNextItemWidth(ImGui.GetContentRegionAvail().X - 150);
            ImGui.InputTextWithHint("##cname", "角色名", ref Selected.Character, 50, C.Censor ? ImGuiInputTextFlags.Password : ImGuiInputTextFlags.None);
...

            {
                ...
                if(ImGui.Button("设置给自己", buttonSize))
                {
    ...
    {
        if(ImGui.BeginCombo("##addnew", "在此选择预设..."))
        {
            ImGuiEx.SetNextItemFullWidth();
            ImGui.InputTextWithHint("##search", "筛选", ref Filter, 50);
            foreach(var x in C.SavedPresets)
