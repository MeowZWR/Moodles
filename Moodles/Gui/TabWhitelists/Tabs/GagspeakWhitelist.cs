...
internal class GagspeakWhitelist : PluginWhitelist
{
    ...
    private void DrawTableForPermissions(MoodlesGSpeakPairPerms whitelistPermissionSet, string id)
    {
        if(ImGui.BeginTable("##wl" + id, 2, ImGuiTableFlags.RowBg | ImGuiTableFlags.SizingFixedFit | ImGuiTableFlags.Borders))
        {
            ...
            ImGui.BeginDisabled();
            ImGui.Checkbox("可以应用我们的Moodles##" + id, ref whitelistPermissionSet.AllowApplyingOwnMoodles);
            ImGui.Checkbox("可以应用他们的Moodles##" + id, ref whitelistPermissionSet.AllowApplyingPairsMoodles);
            ImGui.EndDisabled();
            ...
            ImGui.TableNextColumn();
            ImGui.BeginDisabled();
            ImGui.Checkbox("可以移除Moodles##" + id, ref whitelistPermissionSet.AllowRemoval);
            ImGui.EndDisabled();
            ImGui.EndTable();
        }
    }
